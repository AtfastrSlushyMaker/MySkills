...
import myskillsLogo from '/logos/myskills-logo-icon.png'
...
const TrainerDashboard = () => {
    ...
    // Filtered sessions for search (All Sessions tab)
    const filteredSessions = sessions.filter(session => {
        const term = searchTerm.toLowerCase()
        return (
            session.skill_name?.toLowerCase().includes(term) ||
            session.category?.name?.toLowerCase().includes(term) ||
                            ...
                            {statsConfig.map((stat, index) => (
                                <div key={index} className="dashboard-stats-card">
                                    <div className="flex items-center justify-between mb-4">
                                        ...
                                        <div className={`text-3xl font-black text-${stat.color}-400`}>{sessions.length}</div>
                                    </div>
                                                ...
                                                {sessions.slice(0, 3).map(session => (
                                                    <div key={session.id} className="relative">
                                                        <Link
                                                            ...
                                                            className="dashboard-session-card group"
                                                        >
                                                            <div className="flex items-start justify-between mb-4">
                                                                <div className="w-4/5 min-w-0">
                                                                    <h3 className="dashboard-session-title group-hover:text-cyan-400" title={session.skill_name}>{session.skill_name}</h3>
                                                                    <p className="text-white/70 text-lg truncate" title={session.date ? formatSessionDate(session.date) : 'No date'}>
                                                                        {session.date ? formatSessionDate(session.date) : 'No date'} | {session.location}
                                                                    </p>
                                                                </div>
                                                                <div className="flex flex-col items-end min-w-0 ml-2">
                                                                    ...
                                                                    <i className="fas fa-arrow-right text-cyan-400 text-2xl group-hover:translate-x-2 transition-transform duration-300" />
                                                                </div>
                                                            </div>
                                    ...
                                    >
                                        <div className="w-3/4">
                                            ...
                                            <div className="text-white/70 text-md truncate">
                                                {session.date ? format(parseISO(session.date), 'PPPp') : 'No date'} | {session.location}
                                            </div>
                                        </div>
